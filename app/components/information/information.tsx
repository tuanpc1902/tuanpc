import Image from 'next/image';
import styled from 'styled-components';

const StyleAvatar = styled.div``;

function Information() {
  return (
    <div className="py-20 select-none">
      <StyleAvatar className="flex items-center justify-center w-20 h-20 mx-auto mb-5 border-4 border-third bg-opacity-10 ">
        <Image
          className="pointer-effect text-white"
          src="/avatar.svg"
          alt="avatar"
          height={20}
          width={20}
        />
      </StyleAvatar>
      <div className="max-w-xl mx-auto mb-10 text-3xl font-bold leading-snug text-center text-white lg:leading-relaxed lg:text-4xl">
        I am a Java web programmer
      </div>
      <div className="flex flex-col justify-center gap-5 mb-10 sm:items-center sm:flex-row">
        <a
          href="https://github.com/tuanpc1902/tuanpc1902"
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center justify-center gap-x-3 px-8 py-4 font-sans font-semibold tracking-wide text-white bg-slate-800 rounded-lg h-[60px] w-full sm:w-[230px] button-effect"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            viewBox="0 0 24 24"
          >
            <path
              d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"
              fill="currentColor"
            ></path>
          </svg>
          View on Github
        </a>
        <a
          href="https://fb.com/tuanpc1902"
          target="_blank"
          className="inline-flex items-center justify-center gap-x-3 px-8 py-4 font-sans font-semibold tracking-wide text-white bg-gradient-primary rounded-lg h-[60px] w-full sm:w-[230px] button-effect"
          rel="noreferrer"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="w-5 h-5"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path d="M2 5a2 2 0 012-2h7a2 2 0 012 2v4a2 2 0 01-2 2H9l-3 3v-3H4a2 2 0 01-2-2V5z"></path>
            <path d="M15 7v2a4 4 0 01-4 4H9.828l-1.766 1.767c.28.149.599.233.938.233h2l3 3v-3h2a2 2 0 002-2V9a2 2 0 00-2-2h-1z"></path>
          </svg>
          <span>Contact me</span>
        </a>
      </div>
    </div>
  );
}
export default Information;
